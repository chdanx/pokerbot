...
Для исторических игр без состава участников используйте `import_archive_xlsx.py`. Такие игры видны в истории и входят в сводку первого сезона, но не влияют на общую и личную статистику, рейтинг, рекорды или серии.

Сначала всегда выполните проверку (она ничего не изменяет):

```bash
docker compose exec api python import_archive_xlsx.py /app/data/poker-kansas.xlsx --room-id 1
```

После проверки повторите команду с `--apply`. Скрипт не добавляет дубликаты, а совпавшие старые обычные игры переводит в архив и отвязывает от них игроков. Для известных сокращений из этой таблицы (`Слава Х`, `Дима Б` и т. п.) он подставляет полные постоянные имена, но не создаёт связей с профилями. Файл Excel нужно заранее скопировать в `./data/` на сервере.

## Сезоны и переменные окружения

Первый сезон охватывает всю историю до 31.05.2025. Затем сезоны определяются автоматически: с 1 июня по 30 ноября и с 1 декабря по 31 мая.
